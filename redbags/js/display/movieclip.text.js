(function ($) {
    $.text = function (option) {
        ...
        $text.loading = function () {
            ...
            var text = App.creat("text", {text: "0", font: "20px Arial", color: "#fff"}).set({
                x: 77,
                y: 120,
                textAlign: "center"
            });
            var starArr = [{scale: .7, x: -5, y: 0}, {scale: .5, x: 40, y: -30}, {scale: 1, x: 100, y: -35}, {
                scale: .7,
                x: 140,
                y: -35
            }, {scale: .5, x: 160, y: 35}];
            for (var i = 0; i < starArr.length; i++) {
                var star = App.creat("bitmap", {img: option.loader.getResult("star")}).set({
                    alpha: 0,
                    x: starArr[i].x,
                    y: starArr[i].y,
                    scaleX: starArr[i].scale,
                    scaleY: starArr[i].scale
                });
        ...
        $text.bg = function (obj) {
            ...
            for (var i = 0; i < 30; i++) {
                ...
                var round = App.creat("bitmap", {img: option.loader.getResult("bg")}).set({
                    x: Math.random() * 1200,
                    y: Math.random() * 1800,
                    scaleX: scale,
                    scaleY: scale
