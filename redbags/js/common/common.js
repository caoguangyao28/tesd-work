    ...
    $.common= function() {
        ...
        $common.methods = {
            ...
            run:function(){
                ...
                $(window).bind("resize",function(){$common.methods.resize(0);});
                //初始化背景素材
                this.load({config:bgMaterial,complete:function(event,loader){
                    loading=new $.text({loader:loader}).loading();
                    stage.addChild(loading.movieclip);
                    loader=null;
                    $common.methods.load({config:material,sound:true,progress:function(num){
                            loading.text.text=num;
                        },complete:function(event,loader){
                            createjs.Tween.get(loading.movieclip).to({alpha:0,y:loading.movieclip.y+50},500,createjs.Ease.sineInOut)
                            .call(function(){
                                //删除loading
                                stage.removeChild(loading.movieclip);
                                loading=null;
                                //定义舞台对象
                                new $.stage(document,{stage:stage,loader:loader,width:maxWidth,height:maxHeight}).init();//初始化舞台
                            });
                        }
                    });
            ...
            load:function(option){
                var loader=App.load(option.config,{
                    ...
                    file:file,
                    //加载完成
                    COMPLETE:function(event){if(option.complete)option.complete(event,loader);return;},
                    //加载中...
        ...
        $common.dev={
            ...
            getLottery:function(callback){
                var result=[{num:0,key:"notLottery"},{num:1+Math.floor(Math.random()*99998),key:"lottery"}];
                var resultObj=result[Math.floor(Math.random()*result.length)];
                try{getLottery(result,callback);}catch(e){callback({num:resultObj.num,type:resultObj.type,key:resultObj.key});};
            }
        };
        return $common;
    };
